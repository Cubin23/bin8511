<style>
    ...
    .product img {
        width: 100%;
        max-width: 600px;
        /* Đảm bảo hình ảnh có chiều rộng tối đa */
        height: auto;
        border-radius: 10px;
    ...
    .add-to-cart button:active {
        ...
        /* Quay lại vị trí ban đầu khi nhấn */
    ...
    <?php if (!empty($dsbl)): ?>
        <ul>
            <?php foreach ($dsbl as $bl): ?>
                <li>
                    <p><strong><?= htmlspecialchars($bl['nguoi_dung'] ?? 'Ẩn danh') ?>:</strong></p>
                    <p><?= nl2br(htmlspecialchars($bl['noi_dung'] ?? '')) ?></p>
                    <p><small><?= htmlspecialchars($bl['created_at'] ?? 'Không xác định') ?></small></p>
                </li>
                <hr>
            <?php endforeach; ?>
